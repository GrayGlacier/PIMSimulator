...
#ifndef __PIM_KERNEL_CHANNELWISE_HPP__
#define __PIM_KERNEL_CHANNELWISE_HPP__
...
class PIMKernelChannelwise
{
  public:
    PIMKernelChannelwise(shared_ptr<MultiChannelMemorySystem> mem, int num_pim_chan, int num_pim_rank)
        : mem_(mem),
    ...
    void addBarrier(int ch_idx);
    void runPIM();
    uint64_t getCycle();
    void parkIn(int ch_idx, int ra_idx);
    void parkOut(int ch_idx, int ra_idx);
    void changePIMMode(dramMode mode1, dramMode mode2, int ch_idx, int ra_idx);
    void addTransactionAll(bool isWrite, int ch_idx, int ra_idx, int bg, int bank, 
                            int row, int col, const std::string tag, BurstType* bst, bool use_barrier = false, int num_loop = 1);
    void addTransactionAll(bool isWrite, int ch_idx, int ra_idx, int bg, int bank, 
    ...
    */
    ...
    void programSrf();
    */
    void programCrf(vector<PIMCmd>& cmds, int ch_idx, int ra_idx);
    ...
    */
    ...
    void executeEltwiseMultipleQs(int dim, pimBankType bank_types, KernelType ktype, int ch_idx, int ra_idx,
